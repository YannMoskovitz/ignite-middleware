...
/**Esse middleware deve receber o usuário já dentro do request e
 * chamar a função next apenas se esse usuário ainda estiver no plano grátis e
 * ainda não possuir 10 todos cadastrados ou se ele já estiver com o plano Pro ativado.  */
...
function checksCreateTodosUserAvailability(request, response, next) {
  ...
  if( user.pro === false ) {
    if( user.todos.length > 10) {
      return response.status(403).json({error: "Free usage limit reached, update to Pro plan to create more tasks!"})
    }
  }
...
function checksTodoExists(request, response, next) {
 ...
 const myUser = users.find((user) => user.username === username);
 const myTodo = myUser.todos.find((todo) => todo.id === todoId);

 if(!myTodo) {
   return response.status(404).json({error: "todo not found!"})
 }
 if(!validate(myTodo.id)) {
  return response.status(404).json({error: "Invalid id, id is not UUID!"})
 }
 if(!myUser){
  return response.status(404).json({error: "User does not exist"})
 }
